...
@section('content')
<?php //echo'<pre>';print_r(old('time_allocated'));echo'</pre>';die; ?>
...
<?php if(isset($project) && !empty($project)){ ?>
	...
	<form id="add_projects1" method="post" action="{{ route('save-edited-project') }}">
		<input type="hidden" name="id" value="{{ $project->id }}">
	<?php }else{ ?>
		<form id="add_projects1" method="post" action="{{ route('save-project') }}">
		<?php } ?>
		@csrf
		...
		<div class="add-project11">
			...
			<div class="right_project12 left2">
				...
				<div class="content-devices hourly-rate d-none"> 
					<input type="text" class="form-control" name="hourly_rate" placeholder="Hourly Rate" value="{{ old('hourly_rate') ? old('hourly_rate') : (isset($project->hourly_rate) ? str_replace('$','',$project->hourly_rate) : '') }}">
				</div>
				...
				<div class="content-devices dates2">
					<div class="inner_content">
						<input type="number" class="form-control" name="time_allocated_hours" placeholder="Time Allocated (Hours)" value="{{ old('time_allocated_hours') ? old('time_allocated_hours') : $hours }}">
						<?php if($hours){ ?><p>Time allocated in hours</p><?php } ?>
					</div>
					<div class="inner_content">
						<input type="number" class="form-control" name="time_allocated_mins" placeholder="Time Allocated (Minutes)" value="{{ old('time_allocated_mins') ? old('time_allocated_mins') : $mins }}">
...
<script type="text/javascript">
	$(document).ready(function(){
		...
		var relations = <?php echo old('assigned_users') ? json_encode(old('assigned_users')) : (isset($project_user_relation[0]) && !empty($project_user_relation[0]) ? json_encode($project_user_relation) : json_encode(array())) ?>;
